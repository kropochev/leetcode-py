from collections import Counter
from functools import cache


class Solution:
    def isScramble(self, s1: str, s2: str) -> bool:
        """
        We can scramble a string s to get a string t using the following
        algorithm:

        If the length of the string is 1, stop.
        If the length of the string is > 1, do the following:
        - Split the string into two non-empty substrings at a random index,
        i.e., if the string is s, divide it to x and y where s = x + y.
        - Randomly decide to swap the two substrings or to keep them in the same
        order. i.e., after this step, s may become s = x + y or s = y + x.
        - Apply step 1 recursively on each of the two substrings x and y.

        Given two strings s1 and s2 of the same length, return true if s2 is
        a scrambled string of s1, otherwise, return false.

        >>> Solution().isScramble("great", "rgeat")
        True
        >>> Solution().isScramble("abcde", "caebd")
        False
        >>> Solution().isScramble("a", "a")
        True
        """

        @cache
        def isMatch(substr: str, start: int) -> bool:
            if not Counter(substr) == Counter(s2[start:start + len(substr)]):
                return False

            if len(substr) == 1:
                return True

            for i in range(1, len(substr)):
                left, right = substr[:i], substr[i:]
                if isMatch(left, start) and isMatch(right, start + len(left)):
                    return True
                if isMatch(right, start) and isMatch(left, start + len(right)):
                    return True

            return False

        return isMatch(s1, 0)


if __name__ == "__main__":
    import doctest

    doctest.testmod()
